...
function collectTextNodes(): Text[] {
  ...
  for (const root of getRoots()) {
    ...
    while (current) {
      ...
      if (parent && value && !isExcluded(parent) && !seen.has(current)) {
        nodes.push(current);
        seen.add(current);
      }
...
function writeStore(store: SnapshotStore): void {
  try {
    ...
    // Storage is optional. Never interfere with the page when it is unavailable or full.
  }
}
...
function captureSnapshot(language: string, sourceByPath: Map<string, string>): void {
  ...
  for (const node of collectTextNodes()) {
    ...
    const source = sourceByPath.get(path) ?? node.nodeValue?.trim() ?? '';
    const translated = node.nodeValue?.trim() ?? '';
    if (source && translated && source !== translated) {
      entries.push({ path, index: 0, source, translated });
    }
  }
...
function restoreSnapshot(language: string): boolean {
  ...
  for (const entry of snapshot.entries) {
    ...
    if (node?.nodeType !== Node.TEXT_NODE || isExcluded(node.parentElement as Element)) continue;
...
export function initializeOfflineTranslationCache(): void {
  ...
  const scheduleCapture = (): void => {
    ...
    saveTimer = window.setTimeout(() => {
      ...
      if (!language || !hasTranslatedClass()) return;
      captureSnapshot(language, sourceByPath);
    }, 700);
  };

  const primeSources = (): void => {
    sourceByPath = new Map(collectTextNodes().map((node) => [getPath(node), node.nodeValue?.trim() || '']));
  };

  const restoreIfPossible = (): void => {
    const language = getLanguage();
    if (!language || language === 'ar') return;
    restoreSnapshot(language);
  };
